...
$select = mysqli_query ($conexao, "select id from usuarios where usuario = '$_SESSION[usuario]'") or die(mysqli_error($conexao));

while($exibe = mysqli_fetch_assoc($select)){
	$idAdm = $exibe['id'];
}
$query = "select * from topicos where idAdm = $idAdm";
		...
		<div class="container">
			...
			<div class="row" style="text-align: center; margin-top: 10px;">
				<div class="col-md-10" style="border-right: solid 1px #DEE1E6;">
					...
					<div class="row">
						<div class="col-md-2">
							<a class="btn btn-success" href="cadastraTopico.php"><i class="fas fa-plus"></i> Novo Tópico</a>							
						</div>
					</div>
					<div style="margin-top: 20px">	
						<?php	
						if($row == 0){	
							echo "<h5 style='text-align: center; margin-top:7%'>Nenhum registro encontrado!</h5>";								
							...
							while($dado = $result -> fetch_array()){
								...
								<div class='row'>
									<div class='col-md-9' style='text-align:left'>
										<a href='#'>".$dado['nome']."</a>										
										<p style='font-size: 12px'>Criado em: ".date("d/m/Y H:i:s", strtotime($dado['data']))."</p>
										<p style='font-size: 10px; margin-top:-18px'>Categoria: ".$dado['categoria']."</p>
									</div>
									<div class='col-md-1' style='text-align:center;'>
										<form action='_usuariosTopico.php' method='POST'>
										<a href='#' class='btn btn-outline-dark' onclick='dev()' title='Gerenciar Usuários' style='margin-top:8px' ><i class='fas fa-users-cog'></i></a>
										</form>
									</div>
									<div class='col-md-1' style='text-align:center'>
										<form action='atualizaTopico.php' method='POST'>
										<button type='submit' name='id' value='".$dado['id']."' class='btn btn-outline-primary' title='Editar Tópico' style='margin-top:8px'><i class='fas fa-edit'></i></button>
										</form>
									</div>
									<div class='col-md-1' style='text-align:center'>
										<form action='_excluirTopico.php' method='POST'>
											<button type='submit' name='id' value='".$dado['id']."' class='btn btn-outline-danger' title='Excluir Tópico' style='margin-top:8px;'><i class='far fa-trash-alt'></i></button>
										</form>
									</div>
								</div>
								<hr style='width: 100%; margin-top: -0px'>
